...
<table class="table">
			<tr>
				<th style="background-color: #fff;padding: 15px;text-align: center;">
					<form action="${pageContext.request.contextPath}/statistics/jbxj/rate/week/${matcode}" method="post" id="form">
					<input type="hidden" name="startDate" value="${startDate}"/>
					<input type="hidden" name="endDate" value="${endDate}"/>
					<input type="hidden" name="flag" value="" id="flag">
					</form>
					<label id="weekdate" style="background-color:#009688;padding: 5px 20px;color:#FFF;margin-right:10px;cursor: pointer;" onclick="prev()">上一周</label>
					<label id="weekdate" style="font-size:18px;background-color:#009688;padding: 5px 20px;color:#FFF;">${startDate} - ${endDate}</label>
					<label id="weekdate" style="background-color:#009688;padding: 5px 20px;color:#FFF;margin-left:10px;cursor: pointer;" onclick="next()">下一周</label>
...
<table class="table" id="expTable">
		...
		<c:forEach items="${reportDatas}" var="datas" varStatus="">
			...
			<tr>
				<th colspan="5"  style="background-color: #D4BFFF;padding: 10PX;font-size: 15PX;letter-spacing: 2PX;">${datas.matName } - 产品监控点<label style="font-weight: bold;color: red;">（统计项目成品巡检项设置）</label></th>
			</tr>
			<tr>
				<th>监控内容</th>
				<th>检测项</th>
		...
		<tr>
				<th colspan="5"  style="background-color: #D4BFFF;padding: 10PX;font-size: 15PX;letter-spacing: 2PX;">中控合格率<label style="font-weight: bold;color: red;">（统计项目通过中控巡检点设置）</label></th>
			</tr>
			<tr>
				<th>名称</th>
				<th>检测项</th>
