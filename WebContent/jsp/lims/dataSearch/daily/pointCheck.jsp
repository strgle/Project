...
function selectItem(node){
    ...
    //alert(params);
    $("#ordList").attr("src","${pageContext.request.contextPath}/lims/dataSearch/daily/checklist?startDate="+startDate+"&endDate="+endDate+"&area="+node.area);
 }
/* function selectFolder(node){
	var url = "${pageContext.request.contextPath}/lims/dataSearch/daily/samplelist";
	//获取车间、装置
	if(node.type=="area"){
...
/* function query(){
	...
	$('#tree1').tree({
		url:"${pageContext.request.contextPath}/api/lims/areaPlants/areas",
		data:param,
		selectItem: selectItem,
