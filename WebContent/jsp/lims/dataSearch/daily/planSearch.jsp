    ...
    <form action="" id="form" target="ordList" method="post">
     ...
     <p>结束日期：<input type="text" name="endDate" style="width: 120px;padding-left: 10px;" value="${endDate}" id="endDate" autocomplete="off"/></p>
      ...
      <label class="layui-form-label">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;状态:</label>
       <select name="status" style="width: 120px;padding-left: 10px;margin-left: 8px;" id="select">
          <option value="">样品状态</option>
          <option value="Rejected">生成报告</option>
          <option value="Draft">确认报告</option>
          <option value="Confirm">审核报告</option>
          <option value="Approved">审批报告</option>
          <option value="Released">归档报告</option>
       </select>
    </form>
    </div>
	<!-- 车间装置列表-->
...
function query(){
	...
	$('#tree1').tree({
		url:"${pageContext.request.contextPath}/api/lims/areaPlants/matReport",
		data:param,
...
function selectItem(node){
	 var url = "${pageContext.request.contextPath}/lims/dataSearch/report/reportPlanList";
	$("#matcode").val(node.id);
	...
	$("#ordList").attr("src","${pageContext.request.contextPath}/lims/dataSearch/report/reportPlanList?"+params);  
}
