...
<div class="panel panel-default" style="margin-bottom: 0px;">
 ...
 <form class="form-inline" action="${pageContext.request.contextPath}/lims/zltj/jy/detailNo">
  ...
  <div class="form-group">
    ...
    <button type="button" class="btn btn-info" style="margin-left: 30px;" onclick="onAdd()">申请不统计申请</button>
  </div>
 </form>
 </div>
</div>
  ...
  <tbody>
   <c:forEach items="${jyDetail}" var="d" varStatus="status">
    <tr>
     ...
     <td nowrap="nowrap">${d.sampdate}</td>
     <td>${d.areaName}</td>
     <td>${d.plant}</td>
     <td>
      ${d.ordno}
     ...
     <td><fmt:formatNumber value="${priceMap[d.ordno]}" pattern="￥0.00" /></td>
     <td>
 ...
 var chk_value = new Array(); 
    ...
    layui.use(['layer'],function(){
     layer.prompt({title: '不统计原因', formType: 2}, function(text, index){
        $.ajax({
              type:"post",
              url:"${pageContext.request.contextPath }/lims/zltj/jy/addSpDetail",
              data:{'selectId':chk_value,'reason':text},
              traditional :true, 
              async: false,
              success:function(data){
               //window.location.replace(window.location.href);
               location.href ="${pageContext.request.contextPath }/lims/zltj/jy/detailNo";
              },
