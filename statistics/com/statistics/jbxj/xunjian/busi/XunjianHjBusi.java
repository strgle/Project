...
public class XunjianHjBusi{
	...
	public List<XjDetailVo> queryProcess(String startTime,String endTime){
		this.xjHZ(startTime, endTime,MatType.Huaju);
		...
		List<XjDetailVo> details = dao.queryListObject(sql, XjDetailVo.class,MatType.Huaju,startTime,endTime);
		return details;
	}
	...
	public void xjHZ(String startTime,String endTime,String matType){
		...
		if(list.isEmpty()) {
			...
			if(matType.equals(MatType.Huaju)) {
			...
			if(report.getStatus().equals("0")) {
				...
				if(tj.plusHours(1).isBefore(LocalDateTime.now())) {
					...
					if(matType.equals(MatType.Huaju)) {
						...
						String sql1 = "select matcode from JBXJ_DS_MAT";
						List<String> mats = dao.queryListValue(sql1, String.class);
	...
	public void xjHjProductHZ(String startTime,String endTime,String matType){
		String sql = "select * from JBXJ_XJ_REPORT where MAT_TYPE=? and START_TIME=? and END_TIME=?";
		List<ReportVo> list = dao.queryListObject(sql,ReportVo.class,matType,startTime,endTime);
		...
		if(list.isEmpty()) {
			...
			List<String> mats = dao.queryListValue(sql1, String.class,"HUAJU");
			for(String matcode:mats) {
				List<XjDetailVo>  proDetials = this.productTj(report,matcode);
				details.addAll(proDetials);
			}
	...
	private List<XjDetailVo> processTj(ReportVo report) {
		...
		List<XjDetailVo> details = dao.queryListObject(sql1, XjDetailVo.class, MatType.Huaju);
		for(XjDetailVo detail:details) {
		...
		List<Map<String,Object>> list2 = dao.queryListMap(sql2, MatType.Huaju,report.getStartTime(),report.getEndTime());
