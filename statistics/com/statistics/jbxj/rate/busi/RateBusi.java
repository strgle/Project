...
public class RateBusi{
	 ...
	 * @return
	 */
	public List<RateReport>  queryDayReport(String startDate,String endDate,String reportType,String matcode) {
		
		//获取需要统计的样品信息
		String sql0 = "select T2.matcode,T2.MATNAME from MATERIALS T2 WHERE T2.MATCODE =?";
		List<Map<String,Object>> mates = dao.queryListMap(sql0,matcode);
		...
		*/
		...
		for(Map<String,Object> map:mates) {
			...
			//获取合格数、优等品数、不合格数、回切数
			List<RateDetail> datas = dao.queryListObject(sql1, RateDetail.class,reportType,MatType.Product,startDate,endDate,matCode);
	 ...
	 * @return
	 */
	public List<ReportModel>  queryReport(String startDate,String endDate,String reportType,String matcode) {
		...
		*/
		...
		for(Map<String,Object> map:mates) {
			...
			*/
			...
			List<MonitorDetail> monitorDetails = dao.queryListObject(sql2, MonitorDetail.class, reportType,MatType.Product,matCode,startDate,endDate);
	...
	public RateReport checkReport(String startDate,String endDate,String reportType,String matCode) {
		...
		List<RateReport> reportList = dao.queryListObject(sql1,RateReport.class, reportType,startDate,endDate,MatType.Product,matCode);
		if(reportList.isEmpty()) {
			RateReport report = new RateReport();
	...
	public RateReport checkProcessReport(String startDate,String endDate,String reportType) {
		...
		//判断统计信息是否存在
		String sql1 = "select * from JBXJ_PROCESS_report where report_type=? and start_time=? and end_time=? and mat_type=?";
		List<RateReport> reportList = dao.queryListObject(sql1,RateReport.class, reportType,startDate,endDate,MatType.Process);
		if(reportList.isEmpty()) {
			RateReport report = new RateReport();
			report.setReportType(reportType);
			report.setStartTime(startDate);
			report.setEndTime(endDate);
			report.setMatType(MatType.Process);
			report.setYear(year);
			report.setTjDate(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
			return report;
		}else {
	...
	private void hz(RateReport report) {
		try {
			...
			String sql2 = "select brand,count(*) cnum from ct_orders ct where ct.sampdate>=to_date(?,'yyyy-MM-dd hh24:mi:ss') and ct.sampdate<=to_date(?,'yyyy-MM-dd hh24:mi:ss') "
					+ " and matcode=? and status='Released' and (ct.batchno like '1%' or ct.batchno like '2%') and ct.batchno not like '%24' "
					+ " group by brand";
	...
	private void reHZ(RateReport report){
		try {
			...
			String sql2 = "select brand,count(*) cnum from ct_orders ct where ct.sampdate>=to_date(?,'yyyy-MM-dd hh24:mi:ss') and ct.sampdate<=to_date(?,'yyyy-MM-dd hh24:mi:ss') "
					...
					+ " and (ct.batchno like '1%' or ct.batchno like '2%') and ct.batchno not like '%24' group by brand";
	...
	private void monitorHz(RateReport report) {
		...
		String sql = "select ip.testcode,ip.analyte_id analyte,a.sort,ip.status,max(a.sinonym) sinonym,count(*) num from ipcoa_tempvalues ip,ct_orders ct,JBXJ_DS_ANALYTE a " + 
						...
						"and (ct.batchno like '1%' or ct.batchno like '2%') and ct.batchno not like '%24' and length(ct.batchno)=10 " + 
	...
	private void processMHz(RateReport report) {
		...
		String sql1 = "select * from JBXJ_XJ_MAT where mat_type=? order by sort";
		List<MonitorDetail> details = dao.queryListObject(sql1, MonitorDetail.class,MatType.Process);
		...
		String sql2 = "select mat.point_id,mat.matcode,mat.testcode,mat.analyte,mat.sort,p.s,count(*) cnum "
				...
				+ " group by mat.point_id,mat.matcode,mat.testcode,mat.analyte,mat.sort,p.s order by mat.sort";
		...
		for(Map<String,Object> map:list2) {
			...
			for(MonitorDetail detail:details) {
				if(detail.getPointId().equals(pointId)&&detail.getTestcode()==testcode&&detail.getAnalyte().equals(analyte)) {
					if("Done".equals(s)) {
	...
	private List<RateBrand> brand(int year,String matCode){
		...
		List<RateBrand> list = dao.queryListObject(sql1,RateBrand.class,MatType.Product,year,matCode);
		
		if(list.isEmpty()) {
			String sql2 = "select max(year) year from JBXJ_RATE_brand where mat_type=? and matcode=?";
			Integer maxYear = dao.queryValue(sql2, Integer.class, MatType.Product,matCode);
			if(maxYear != null) {
				String sql3 = "select * from JBXJ_RATE_brand where year=? and mat_type=? and matcode=?";
				list = dao.queryListObject(sql3, RateBrand.class, maxYear,MatType.Product,matCode);
