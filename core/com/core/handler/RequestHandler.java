...
public class RequestHandler extends HandlerInterceptorAdapter {
	 ...
	 * 第三个参数为handler响应的处理器（如Controller的实现）
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(session!=null){
			Object userName = session.getAttribute("usrNam");
			if(userName==null){
				if(uri.contains("screen")){
					return super.preHandle(request, response, handler);
				}else if(uri.contains("/dataSearch/daily/ordDetail")){
					return super.preHandle(request, response, handler);
				}else{
		...
		}else if(request.getParameter("loginKey")!=null){
			//
		}else{
			if(uri.contains("screen")){
				return super.preHandle(request, response, handler);
